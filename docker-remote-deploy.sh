...
ssh -o StrictHostKeyChecking=no -i $PRIVATE_KEY_FILE $DOCKER_HOST_USER@$DOCKER_HOST "docker run -d --name $CONTAINER_NAME -p $CONTAINER_PORTS $CONTAINER_ARGS $IMAGE_NAME:$TAG_NAME"
